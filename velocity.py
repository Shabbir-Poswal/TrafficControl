...
# Previous frame for calculating vehicle speed
prev_frame = None
    ...
    if ret:
        ...
        for box in bounding_boxes.xyxy:
            ...
            if prev_frame is not None:
                prev_bounding_boxes = best_model.predict(prev_frame, imgsz=640, conf=0.2)[0].boxes
                for prev_box in prev_bounding_boxes.xyxy:
                    if vehicle_id == tuple(prev_box.cpu().detach().numpy().astype(int)):
                        # Calculate displacement
                        displacement = ((box[0] - prev_box[0]) ** 2 + (box[1] - prev_box[1]) ** 2) ** 0.5
                        # Calculate speed (assuming constant frame rate)
                        speed = displacement * cap.get(cv2.CAP_PROP_FPS)
                        # Display speed on the frame
                        cv2.putText(processed_frame, f'Speed: {speed:.2f} pixels/frame', 
                                    (int(box[0]), int(box[1]) - 20), font, font_scale, font_color, 2, cv2.LINE_AA)
